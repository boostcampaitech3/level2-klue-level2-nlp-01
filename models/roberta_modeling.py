...
from transformers.models.roberta.modeling_roberta import RobertaPreTrainedModel, RobertaEncoder, \
    RobertaClassificationHead, create_position_ids_from_input_ids
...
class RobertaEmbeddingwithEntity(nn.Module):
    ...
    def __init__(self, config):
        ...
        # KBS) Added Entity embeddings
        self.entity_embeddings = nn.Embedding(9, config.hidden_size)
    ...
    def forward(
            self, input_ids=None, token_type_ids=None, position_ids=None, entity_ids=None, inputs_embeds=None,
            past_key_values_length=0
    ):
        if entity_ids is None:
            raise Exception("entity_ids를 입력해주세요!")
...
class RobertaModelWithEntityEmbeddings(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]

    def __init__(self, config, add_pooling_layer=True):
        super().__init__(config)
        self.config = config
        # KBS) modified embeddings
    ...
    def forward(
        ...
        if entity_ids is None:
            raise Exception("entity ids를 입력해주세요!")
...
class RobertaForKlueReTask(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
...
class RobertaNotUsingClsForKlueReTask(RobertaPreTrainedModel):
    _keys_to_ignore_on_load_missing = [r"position_ids"]
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        subj_idx = ((entity_ids == 1) + (entity_ids == 2)).nonzero(as_tuple=True)[0]
        obj_idx = (entity_ids >= 3).nonzero(as_tuple=True)[0]
        logits = self.classifier(sequence_output, subj_idx, obj_idx)
