from transformers import AutoTokenizer, AutoConfig, AutoModelForSequenceClassification, Trainer, TrainingArguments, RobertaForSequenceClassification
...
def inference(model, tokenized_sent, device):
  ...
  """
  dataloader = DataLoader(tokenized_sent, batch_size=4, shuffle=False)
  model.eval()
  output_pred = []
  output_prob = []
...
def load_test_dataset(dataset_dir, tokenizer):
  ...
  # tokenizing dataset
  tokenized_test = tokenized_dataset_custom_tagging(test_dataset, tokenizer)
...
def main(args):
  ...
  output_path = os.path.join(args.model_dir, 'submission.csv')
